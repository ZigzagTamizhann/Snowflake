...
motor = Motor(a1_pin=Snowflake.IO1, a2_pin=Snowflake.IO2, b1_pin=Snowflake.IO3, b2_pin=Snowflake.IO4, speed=0.2)
...
try:
    ...
    while True:
        ...
        # Case 4: Both sensors on black line -> Initiate Sequential Parking Maneuver
            ...
            motor.turn_left()
            time.sleep_ms(500) # Adjust for a 90-degree turn
            motor.turn_left()
            time.sleep_ms(500) # Adjust for a 90-degree turn
            motor.stop()
            time.sleep_ms(500) # Adjust for a 90-degree turn
            ...
            if count == 3:
                ...
                l, r = ir_sensor.read_line()
                while l == 1 and r == 1:
                    print(f"Sensor values: Left={l}, Right={r}")
                    motor.stop()
